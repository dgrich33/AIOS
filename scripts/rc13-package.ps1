...
if (-not (Test-Path (Join-Path $Root "license.cert"))) {
  if (Test-Path (Join-Path $Root ".venv\Scripts\python.exe")) {
    & (Join-Path $Root ".venv\Scripts\python.exe") (Join-Path $Root "scripts\generate_license.py") | Out-Host
  } else {
    python (Join-Path $Root "scripts\generate_license.py") | Out-Host
  }
}

$Zip = "C:\AIOS\aios-codex-unlimited-enterprise-v2-RC13.zip"
$StagingRoot = Join-Path $ReleaseDir "zip-staging-rc13"
...
Get-ChildItem -LiteralPath $Root -Recurse -File -Force | Where-Object {
  ...
  -not (
    ...
    $_.Name -in @(".env","auth.json","credentials.json","aios_dev.db","aios_dev.db-wal","aios_dev.db-shm","test_aios.db","test_aios.db-wal","test_aios.db-shm") -or
